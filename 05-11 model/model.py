...
class ProdLDA(nn.Module):
    def __init__(self, vocab_size, num_topics, num_prototypes, hidden, dropout, device, frozen=False):
        ...
        # priors on the prototype vectors; shouldnt be called log'theta'
        self.logtheta_loc_p = torch.zeros(self.num_prototypes, self.num_topics, device=self.device)
        self.logtheta_scale_p = 10 * torch.ones(self.num_prototypes, self.num_topics, device=self.device)
        self.prototypes = torch.nn.Parameter(torch.randn(self.num_prototypes, self.num_topics, device=self.device))
    ...
    def guide(self, docs_a, docs_b, c, observed, x=None):
        ...
        p = pyro.param('p', self.prototypes)
...
def custom_elbo(model, guide, *args, **kwargs):
    ...
    sim_loss_weight = 5
    # c = model_trace.nodes.pop('obs_c')
    # sim_loss = c['log_prob_sum']

    
    guide_term = guide_trace.log_prob_sum()
    model_term = model_trace.log_prob_sum()
    sim_loss = model_trace.nodes.pop('obs_c')['log_prob_sum']
    model_term = (model_term - sim_loss) + sim_loss_weight * sim_loss
    # model_term = model_trace.log_prob_sum() + sim_loss_weight * sim_loss
